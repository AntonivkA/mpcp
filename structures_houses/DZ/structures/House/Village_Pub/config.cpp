...
class CfgVehicles
{
	...
	class Land_Village_pub: HouseWithFireplace
	{
		...
		class UserActions
		{
			...
			{
				position = doorstwin_action;
				condition = ((this animationSourcePhase 'doorstwin') >= 0.5);
				statement = (this animateSource ['doorstwin', 0]);
			};
			class CloseDoors_5: CloseDoors_1
			{
				position = doorstwin_action;
				condition = ((this animationSourcePhase 'doorstwin') < 0.5);
				statement = (this animateSource ['doorstwin', 1]);
			};
		};
	};
};
