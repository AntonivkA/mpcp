...
{	
	...
	{ 
		...
		*/
		if (
			(count (itemsInInventory _fireplace) == 0) and
			(_fireplace animationPhase 'LidOn' == 0)) then
		{
			true
		}
		else
	...
	case 8:
	{
		...
		if (
			(_fireplace animationPhase 'LidOn' == 0) and 
			(isNull _cooking_equipment) and
			(isNull itemParent this)) then
		{
			true
		};
	};
	...
	case 9:
	{
		...
		if (
			(_fireplace animationPhase 'LidOff' == 0) and
			(isNull itemParent this)) then
		{
			true
		};		
