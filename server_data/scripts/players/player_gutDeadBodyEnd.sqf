...
// Definition of items which are gathered from the dead bodies
_gainedFood = []; // Types of meats. Their base quantity is 100% which is affected by the tool's damage. 2nd parameter (the number) represents the count of this type of items
_gainedItems = [];// Types of items which are usable in crafting. Their condition is affected by the tool's damage. 2nd parameter (the number) represents item's quantity.
_gainedStack = [];// Stack of items. They are always pristine. The 2nd parameter (the number) represents the item's quantity.
...
// Adding organs to the player's inventory
if (count _gainedFood == 0 && count _gainedItems == 0 && count _gainedStack == 0) then 
{
