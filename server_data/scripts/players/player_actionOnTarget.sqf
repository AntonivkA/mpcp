ProfileStart "player_actionOnTarget.sqf";
private["_state"];
/*
...
switch _state do 
{
	case 0:
	{
		...
		*/
		...
		//_person lockItemToHands _item;
		_person setSlotLock ["Hands", true];
		diag_log format ["OnTarget:::Lock::: %1", _person getSlotLock "Hands" ];
		...
		*/
		...
		if (_canUseActions == 0) exitWith
		{
			...
			//_person unlockItemFromHands _inUse;
			...
			_person setVariable ["wasCanceled",0];
			ProfileStop "player_actionOnTarget.sqf";
		};
	...
	case 1:
	{
		...
		*/		
		...
		if (_actionCanceledDoubleCheck == 1) exitWith
		{
			...
			//_target setVariable ["inUseItem",objNull];
			//_person unlockItemFromHands (itemInHands _person);
			_person setSlotLock ["Hands", false];
			diag_log format ["OnTarget:::Lock::: %1", _person getSlotLock "Hands" ];
		...
		[_target,_item,_scale] call player_fnc_processStomach; // process any nutritional benefits
		...
		*/
		if(!_unlimited) then {
			...
			if (!_keepEmpty and quantity _oldItem <= 0) then {
				//_person unlockItemFromHands _oldItem;
				_person setSlotLock ["Hands", false];
				diag_log format ["OnTarget:::Lock::: %1", _person getSlotLock "Hands" ];
