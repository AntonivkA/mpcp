	...
	//fill database with amount of pelts
	{
		if((_x isKindOf 'Consumable_GardenLime') and ((damage _x) < 1))then{
			...
			if((_x isKindOf 'Consumable_PeltBase') and ((damage _x) < 1))then{
				...
				if(_x isKindOf 'Consumable_PeltWolf')then		{if((_tancAr select 2) < _minimumlime)then{_minimumlime = (_tancAr select 2);};_peltAr set [2,((_peltAr select 2)+1)];_peltcount = (_peltcount+(_tancAr select 2));};				
			};
		};
	} forEach _cargo;
	...
	//delete used pelts
	{
		if(_x isKindOf 'Consumable_GardenLime')then{
			...
			if(_x isKindOf 'Consumable_PeltBase')then{
				...
				if((_x isKindOf 'Consumable_PeltWolf') and ((_peltAr select 2) > 0))then		{_peltAr set [2,((_peltAr select 2)-1)];deleteVehicle _x;};				
			};
		};
	} forEach _cargo;
