ProfileStart "player_actionOnSelf.sqf";
private["_state"];
/*
	...
	case 0:
	{
		...
		*/
		...
		if (_canUseActions == 0) exitWith
		{
			...
			diag_log format ["OnSelf:::Lock::: %1", _person getSlotLock "Hands" ];
			
			_person setVariable ["inUseItem",objNull];
			_person setVariable ["isUsingSomething",0];
			_person setVariable ["wasCanceled",0];
			ProfileStop "player_actionOnSelf.sqf";
		};
		/*
		...
		*/
		...
		if (_inHands != _item) exitWith
		{
			...
			if (!isNull _inHands) then
			{
				...
				_person moveItemFromHandsToInventory _inHands;
	...
	case 1:
	{
		...
		*/
		...
		if (_actionCanceledDoubleCheck == 1) exitWith
		{
			...
			//_person unlockItemFromHands _item;
			_person setSlotLock ["Hands", false];
			diag_log format ["OnSelf:::Lock::: %1", _person getSlotLock "Hands" ];
		...
		[_person,_item,_scale] call player_fnc_processStomach; // process any nutritional benefits			
