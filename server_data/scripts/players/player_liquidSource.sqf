...
diag_log format ["LiquidSource:::Lock::: %1", _person getSlotLock "Hands" ];
...
switch _sourceType do
{
	case "water" :
	{
		if ( isNull _bottle ) then
		{
			...
			{
				_source call pumpAnimation;
				_person playAction ['drinkWell',{_this setVariable ['isUsingSomething',0];_this setSlotLock ["Hands", false];diag_log format ["LiquidSource:::Lock::: %1", _this getSlotLock "Hands" ];}];	
			...
			{
				_person playAction ['drinkPond',{_this setVariable ['isUsingSomething',0]; _this setSlotLock ["Hands", false];diag_log format ["LiquidSource:::Lock::: %1", _this getSlotLock "Hands" ];}];
