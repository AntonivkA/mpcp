	...
	{	
		...
		if (_inArray) then
		{
			...
			if (!isNil "_value") then
			{
				...
				_cFormerDuration = (_cState select 1);
				_cDuration = (_cState select 1) * _value;
				_cState set [1,_cDuration];
				_modstates set [_i,_cState];
				_doSave = true;
				
				//diag_log format ["DURATION_CHANGE>> modifier: %1, current duration: %2 shortened by %3 = new duration: %4",_modifier,_cFormerDuration,_value,_cDuration];
			}
			else
			{
				diag_log format["ERROR: Bad data in _value %2 passed to event_modifier %1",_modifier,_value];
			};
		}
		else
		{
